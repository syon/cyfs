...
async function filterByStatDate(srcList, dateOpts) {
  ...
  if (after) {
    list = await filter(list, async fp => {
      const time = await util.getFileTimestamp(fp, mode)
...
function validateOrder(userOrder) {
  ...
  if (order.select.include) {
    if (order.select.include.preset) {
      if (order.select.include.preset === "emptydir") {
        if (globOpts.nodir) {
          // TODO: blankdir needs `nodir: false` -> needs warning
...
module.exports = async order => {
  let list = []
  const { select: query } = validateOrder(order)
