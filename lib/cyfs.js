...
module.exports = class Cyfs {
  async init(order) {
    if (!order.select || !order.select.pattern) {
      throw new Error()
    }
    this.list = await listing(order)
  }
  ...
  async report(args, isPreview) {
    const fmt = args.timestampFormat || moment.HTML5_FMT.DATETIME_LOCAL
    const entries = await map(this.list, async fp => {
      ...
      return {
        ...
        exif_date: moment(ed).format(fmt),
      }
    })
